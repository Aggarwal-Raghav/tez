...
test('Custom valueMin & valueMax', function(assert) {
  ...
  notOk(this.$('.striped')[0], "Striped class added");
});
...
test('Check for stripes & animation while in progress', function(assert) {
  ...
  equal(this.$().text().trim(), '50%');
  ok(this.$('.striped')[0], "Striped class added");
  ok(this.$('.animated')[0], "Animated class should be added!");
});
