...
test('Basic creation test', function(assert) {
  ...
  ok(model.runningTasks);
  ok(model.pendingTasks);
...
test('runningTasks test', function(assert) {
  ...
  Ember.run(function () {
    equal(model.get("runningTasks"), null);
    model.set("status", "SUCCEEDED");
    equal(model.get("runningTasks"), 0);
  });
});
...
test('pendingTasks test', function(assert) {
  ...
  Ember.run(function () {
    ...
    equal(model.get("pendingTasks"), null);
    model.set("totalTasks", 2);
    model.set("_succeededTasks", 1);
    model.set("status", "SUCCEEDED");
    equal(model.get("pendingTasks"), 1);
  });
});
...
test('description test', function(assert) {
  ...
  equal(model.get("description"), undefined);
  ...
  Ember.run(function () {
    ...
    equal(model.get("description"), testDesc);
  });
});
