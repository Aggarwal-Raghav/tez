...
test('txt: String', function(assert) {
  equal(txt(["Abc"], {}), "Abc");
  equal(txt(null, {}).string, '<span class="txt-message"> Not Available! </span>');
});
...
test('txt: String - error', function(assert) {
  ...
  obj.toString = null;
  equal(txt([obj], {}).string, '<span class="txt-message"> Invalid Data! </span>');
});
...
test('txt: error', function(assert) {
  ...
  equal(txt([err], {}).string, '<span title="testError" class="txt-message"> testError </span>');
});
