        ...
        CuratorCacheListener.builder()
            .forCreates(
                node -> {
                  ...
                  String jsonStr = new String(node.getData(), java.nio.charset.StandardCharsets.UTF_8);
                  String host;
                  int shufflePort = 0;
                  try {
                    org.codehaus.jettison.json.JSONObject json = new org.codehaus.jettison.json.JSONObject(jsonStr);
                    host = json.getString("host");
                    shufflePort = json.getInt("shufflePort");
                  } catch (org.codehaus.jettison.json.JSONException e) {
                    LOG.warn("Failed to parse JSON payload from ZK, falling back to raw string. Payload: {}", jsonStr);
                    host = jsonStr;
                  }

                  LOG.info("Discovered new TezChild via ZK: {} on host {} with shufflePort {}", containerIdStr, host, shufflePort);

                  try {
                    ContainerId cId = ContainerId.fromString(containerIdStr);
                    NodeId mockNodeId = NodeId.newInstance(host, shufflePort);
